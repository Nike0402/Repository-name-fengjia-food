...
export default function RestaurantDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="detail-shell"><div className="detail-container"><Link className="back-link" href="/">← 返回餐廳列表</Link><div className="detail-hero"><div className="detail-image" style={restaurant.imageUrl ? { backgroundImage: `url(${restaurant.imageUrl})` } : undefined}>{!restaurant.imageUrl && restaurant.name.slice(0, 1)}</div><div className="detail-copy"><p className="kicker">{labels[restaurant.category] ?? restaurant.category} / FENGJIA</p><h1>{restaurant.name}</h1><p className="detail-description">{restaurant.description ?? "逢甲商圈值得收藏的用餐選擇。"}</p><div className="detail-rating"><strong>★ {restaurant.rating?.toFixed(1) ?? "—"}</strong><span>價位 {restaurant.priceRange ?? "—"}</span></div></div></div><section className="info-grid"><div><span>地址</span><p>{restaurant.address}</p></div><div><span>電話</span><p>{restaurant.phone ? <a href={`tel:${restaurant.phone}`}>{restaurant.phone}</a> : "尚未提供"}</p></div></section><div className="detail-actions">{restaurant.googleMapsUrl && <a className="primary-link" href={restaurant.googleMapsUrl} target="_blank" rel="noreferrer">開啟 Google Maps ↗</a>}{restaurant.menuUrl && <a href={restaurant.menuUrl} target="_blank" rel="noreferrer">查看菜單</a>}{restaurant.orderUrl && <a href={restaurant.orderUrl} target="_blank" rel="noreferrer">線上訂餐</a>}{restaurant.websiteUrl && <a href={restaurant.websiteUrl} target="_blank" rel="noreferrer">官方網站</a>}</div></div></main>;
}